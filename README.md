...
You do not need to use Conda. Simply clone the repository and run:
!pip install -r requirements.txt
...
Implement `get_sds_loss()` in `guidance/sd.py`. The function receives the latent image $\mathbf{x^{0}}$, text embedding $c$, and optionally, `guidance_scale` used for the Classifier Free Guidance weight. The function should return the computed loss of SDS. 
...
Variational Score Distillation (VSD) in ProlificDreamer aims to improve the sampling quality of SDS by utilizing LoRA to mimic the noise prediction of a pre-trained diffusion model. Given the pretrained diffusion model and a LoRA module, denoted as $\phi$, VSD optimizes the following loss:
...
Task 1 and Task 2 are worth 50 points each, while Task 3 (Optional) is worth 10 points.
...
#### Plagiarism in any form will also result in a zero score.
